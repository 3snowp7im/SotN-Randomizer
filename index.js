(function() {
  ...
  function optionsFromUrl(url) {
    url = new URL(url)
    return {
      seed: url.searchParams.get('s'),
      options: optionsFromString(url.searchParams.get('r') || defaultOptions),
    }
  }

  function optionsToUrl(options, seed, baseUrl) {
  ...
  if (isNode) {
    ...
    const randomizeItems = require('./items')
    const randomizeRelics = require('./relics')
    ...
    if (argv.url && !argv._[1]) {
      console.log(optionsToUrl(options, seed, baseUrl).toString())
    }
    if (argv.verbose >= 1) {
