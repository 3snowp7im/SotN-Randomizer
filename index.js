...
function randomizeRelics(data, options) {
  ...
  if (options.relicLocations) {
    ...
    // game without a speedrun trick. In a randomized relic run, however, it is
    // possible to have early movement options that trigger this softlock for
    // unwitting players. To be safe, disable the cutscene from ever taking
    // place.
    // The flag that gets set after the Maria cutscene is @ 0x3be70.
    ...
    // place.
    // The flag that gets set after the Maria cutscene is @ 0x3ca90.
    // The instruction that checks that flag is:
    // 0x108358:     bne r2, r0, 0xf0914     14400004
    // Change the instruction so it always branches:
    // 0x108358:     beq r0, r0, 0xf0914     10000004
    data[0x108358 + 2] = 0x00
    data[0x108358 + 3] = 0x10
  }
}
