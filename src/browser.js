(function(window) {
  ...
  function presetIdChange() {                                                                       // auto checks modes and options that presets use
    ...
    if (["boss-rush", "beyond"].includes(preset.id)) {                                              // Remove starting room rando if the preset is Boss Rush or Beyond- eldri7ch
      elems.startRoomRandoMode.checked = false
      elems.startRoomRandoMode.disabled = true
    } else {
      elems.startRoomRandoMode.disabled = false
    }
    if (["boss-rush"].includes(preset.id)) {                                                        // Remove unlocked mode if the the preset is Boss Rush. - crazy4blades
