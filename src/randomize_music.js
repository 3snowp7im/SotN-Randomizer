(function(self) {
  ...
  function randomizeMusic(rng, options) {
    ...
    const allMusic = constants.MUSIC
    const bossMusic = constants.BOSS_MUSIC
    const travelMusic = constants.TRAVEL_MUSIC
    
    if (options.music) {
      let music
      
      // Removed the Music by Song option because no one wants that or even knows it exists. - eldri7ch
      ...
      function randomizeMusicByArea(rng,music,list) {
        let songSrc
      ...
      if (options.bossMusicSeparation) {
        ...
        music = Object.values(travelMusicByArea)
        randomizeMusicByArea(rng,music,travelMusic)
      } else {
        music = Object.values(musicByArea)
        randomizeMusicByArea(rng,music,allMusic)
      }
    }
